...
- **Foco**: API testing e documentação de processos  
...
- **E-Commerce Core API**  
  - **Domínios principais**: produtos (CRUD), clientes, vendas, itens de venda, integração ViaCEP  
  - **Migrações**: Liquibase com changelogs em YAML para evolução controlada do schema  
  - **Perfis**: `homolog` (H2) e `prod` (MySQL) com boas práticas de configuração  
  - **Arquitetura**: monolito modularizado por domínio; camadas controller → service → repository → entity  
  - **Testes**: JUnit 5 + Mockito para unit e integração; integração futura com JaCoCo e Checkstyle  
  - **Operacional**: exemplos de CI (GitHub Actions), planos de Dockerização e deploy automatizado no roadmap
